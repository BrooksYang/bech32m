...
function convertBits(array $data, $inLen, $fromBits, $toBits, $pad = true)
{
    ...
    for ($i = 0; $i < $inLen; $i++) {
        ...
        if ($value < 0 || $value >> $fromBits) {
            throw new Bech32mException('Invalid value for convert bits');
        }
    ...
    if ($pad) {
        ...
        throw new Bech32mException('Invalid data');
    }
 ...
 * @return int[]
 */
function createChecksum($hrp, array $convertedDataChars, $enc)
{
    $values = \array_merge(hrpExpand($hrp, \strlen($hrp)), $convertedDataChars);
    $polyMod = polyMod(\array_merge($values, [0, 0, 0, 0, 0, 0]), \count($values) + 6) ^ getEncodingConst($enc);
    $results = [];
    for ($i = 0; $i < 6; $i++) {
 ...
 * @return string
 */
function encode($hrp, array $combinedDataChars, $enc)
{
    $checksum = createChecksum($hrp, $combinedDataChars, $enc);
 ...
 */
function decodeRaw($sBech, $enc)
{
    $length = \strlen($sBech);
    if ($length < 8) {
        throw new Bech32mException("Bech32 string is too short");
    }
    ...
    for ($i = 0; $i < $length; $i++) {
        $x = $chars[$i];
        if ($x < 33 || $x > 126) {
            throw new Bech32mException('Out of range character in bech32 string');
        }
    ...
    if ($haveUpper && $haveLower) {
        throw new Bech32mException('Data contains mixture of higher/lower case characters');
    }

    if ($positionOne === -1) {
        throw new Bech32mException("Missing separator character");
    }

    if ($positionOne < 1) {
        throw new Bech32mException("Empty HRP");
    }

    if (($positionOne + 7) > $length) {
        throw new Bech32mException('Too short checksum');
    }
    ...
    if (!verifyChecksum($hrp, $data, $enc)) {
        throw new Bech32mException('Invalid bech32 checksum');
    }
 ...
 * @throws Bech32mException
 */
function decode($sBech, $enc)
{
    $length = strlen($sBech);
    if ($length > 90) {
        throw new Bech32mException('Bech32 string cannot exceed 90 characters in length');
    }

    return decodeRaw($sBech, $enc);
}
...
function validateWitnessProgram($version, $program)
{
    if ($version < 0 || $version > 16) {
        throw new Bech32mException("Invalid witness version");
    }

    $sizeProgram = strlen($program);
    if ($version === 0) {
        if ($sizeProgram !== 20 && $sizeProgram !== 32) {
            throw new Bech32mException("Invalid size for V0 witness program");
        }
    }

    if ($sizeProgram < 2 || $sizeProgram > 40) {
        throw new Bech32mException("Witness program size was out of valid range");
    }
}
 ...
 * @throws Bech32mException
 */
function encodeSegwit($hrp, $version, $program, $enc)
{
    $version = (int) $version;
    ...
    return encode($hrp, $encodeData, $enc);
}
 ...
 * @throws Bech32mException
 */
function decodeSegwit($hrp, $bech32, $enc)
{
    [$hrpGot, $data] = decode($bech32, $enc);
    if ($hrpGot !== $hrp) {
        throw new Bech32mException('Invalid prefix for address');
    }

    $dataLen = count($data);
    if ($dataLen === 0 || $dataLen > 65) {
        throw new Bech32mException("Invalid length for segwit address");
    }
